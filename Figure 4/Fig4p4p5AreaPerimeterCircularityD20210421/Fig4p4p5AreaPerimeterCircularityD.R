...
#############################################
nls.out = nls(Perimeter~b*Area^z,start=c(b=10,z=0.5), data=CrackPlant)
summary(nls.out)
FS=16
...
p <- ggplot(CrackPlant, mapping=aes(x=Area, y=Perimeter, alpha=1, color=Circularity,stroke=FALSE)) +
  ...
  annotate("text", x=10, y=5, label=TeX("$y = 0.69x^{0.53}$", output="character"),
...
p1
ggsave(p1,filename = "Fig4p4AreaPerimeterD.pdf",width = 5,height = 5)
...
pp <- ggplot(CrackPlant, aes(x=Area, y=Circularity, alpha=1, color=Perimeter)) +
  ...
  scale_x_continuous(limits=c(1,100), breaks=c(1,25,50,75,100))+
...
pp1
ggsave(pp1,filename = "Fig4p5AreCircularityD.pdf",width = 5,height = 5)
