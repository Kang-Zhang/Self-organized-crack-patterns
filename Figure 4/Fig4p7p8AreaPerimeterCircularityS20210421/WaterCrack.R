...
library(readxl)
## ÔØÈëÊı¾İ
CrackPlant <- read_excel("WaterCrack.xlsx", 'Sheet1')
# CrackPlant
head(CrackPlant)
#CrackPlant = CrackPlant[which(CrackPlant$Perimeter<200),]
# classic plot :
p <- ggplot(CrackPlant, aes(x=Length, y=Width, color=Direction)) +
     geom_point(size=5) +
...
ggsave(p1,filename = "WaterCrack.pdf",width = 5,height = 5)
